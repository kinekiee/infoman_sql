...
CREATE TABLE book_copy (  --  individual copies of books available at different branches, Use Copy Number Instead of Auto Increment for Unique Identification of Each Copy    
    book_id         INT             NOT NULL, -- copy_id to book_id for reference to the book
    copy_number     INT             NOT NULL, -- unique identifier for each copy of a book
    branch_id       INT             NOT NULL,
    copy_condition  ENUM('New','Good','Fair','Poor') NOT NULL DEFAULT 'Good',

    PRIMARY KEY (book_id, copy_number, branch_id), -- composite primary key for unique ID of each copy across branches

    CONSTRAINT chk_copy_number CHECK (copy_number > 0), -- ensure copy number is positive
...
CREATE TABLE loan_item (
    ...
    book_id         INT             NOT NULL, -- book_copy_id to book_id for reference to the book
    copy_number     INT             NOT NULL, -- unique identifier for each copy of a book
    branch_id       INT             NOT NULL, -- reference to the branch where the copy is located
    return_date     DATE,
    status          ENUM('On loan','Returned','Overdue') NOT NULL DEFAULT 'On loan',

    PRIMARY KEY (loan_id, book_id, copy_number, branch_id), -- composite primary key to uniquely identify each loan item
    ...
    CONSTRAINT fk_loanitem_copy
        FOREIGN KEY (book_id, copy_number, branch_id) -- reference to the specific copy of the book being loaned
...
INSERT INTO book_copy (book_id, copy_number, branch_id, copy_condition) VALUES -- copy_number added for unique identification of each copy of a book across branches
(1, 1, 1, 'New'),
(1, 2, 1, 'Good'),
(1, 3, 1, 'Fair'),
(1, 1, 2, 'New'),
...
INSERT INTO loan (member_id, loan_date, due_date) VALUES -- loan_id, member_id, loan_date, due_date
...
INSERT INTO loan_item (loan_id, book_id, copy_number, branch_id, return_date, status) VALUES -- loan_id, book_copy_id, copy_number, branch_id for reference to the specific copy of the book being loaned, return_date, status
